...
NFmiODBC::NFmiODBC(const std::string& user, const std::string& password, const std::string& database)
    : NFmiDatabase(user, password, database), id_(0)
{
}

void NFmiODBC::Connect(const string& user, const string& password, const string& database, const int threadedMode)
{
...
void NFmiODBC::Connect() { return Connect(1); }
...
{
	if (!connected_) throw runtime_error("NFmiODBC: Cannot perform SQL query before connected");
...
void NFmiODBC::Execute(const string& sql) throw(int)
{
	...
	try
	{
		odbc::otl_cursor::direct_exec(db_, sql.c_str(), odbc::otl_exception::enabled);
	}
	catch (odbc::otl_exception& p)
	{
		// re-throw error code
