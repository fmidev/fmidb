...
NFmiPostgreSQL::NFmiPostgreSQL(const std::string& user, const std::string& password, const std::string& database, const std::string& hostname, int port)
...
void NFmiPostgreSQL::Connect(const string & user,
...
#ifdef DEBUG
	cout << "DEBUG: dsn is 'user="<<user_<<" password=xxx host="<<hostname_<<" dbname="<<database_<<" port="+boost::lexical_cast<string> (port_) << "'" << endl;
#endif

	connection_string_ = "user="+user_+" password="+password_+" host="+hostname_+" dbname="+database_+" port="+boost::lexical_cast<string> (port_);
...
vector<string> NFmiPostgreSQL::FetchRow() {
	...
	if(!connected_)
		throw runtime_error("NFmiPostgreSQL: Cannot perform SQL query before connected");
