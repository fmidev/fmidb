...
NFmiVerifDB::NFmiVerifDB() : NFmiPostgreSQL()
{
	connected_ = false;
	user_ = "verifimport";
	password_ = "08y8abei";
...
NFmiVerifDB::~NFmiVerifDB() { Disconnect(); }
/*
...
map<string, string> NFmiVerifDB::GetStationInfo(unsigned long station_id, bool aggressive_cache)
{
	...
	if (stations.find(station_id) != stations.end()) return stations[station_id];
...
{
	if (producerinfo.find(producer) != producerinfo.end()) return producerinfo[producer];
...
map<int, map<string, string> > NFmiVerifDB::GetStationListForArea(unsigned long producer_id, double max_latitude,
...
{
	if (!metadata.instantiated) Initialize();
	...
	if (stat == "FARE") stat = "FAR";

	if (metadata.statIds.find(theStat) != metadata.statIds.end()) return metadata.statIds[theStat];
...
int NFmiVerifDB::PeriodId(const string &thePeriodName)
{
	...
	if (metadata.periodIds.find(thePeriodName) != metadata.periodIds.end()) return metadata.periodIds[thePeriodName];
	...
	string sql = "insert into periods (period,start_date,end_date) values ('" + parts[0] + "','" + parts[1] + "','" +
	             parts[2] + "')";

	Execute(sql);

	sql = "SELECT id FROM periods WHERE period = '" + parts[0] + "' AND start_date = '" + parts[1] +
	      "' AND end_date = '" + parts[2] + "'";
...
void NFmiVerifDB::Initialize(void)
{
	...
	Query(
	    "SELECT id,period||','||to_char(start_date, 'yyyy-mm-dd')||','||to_char(end_date, 'yyyy-mm-dd') FROM "
	    "verifng.periods");
