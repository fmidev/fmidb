...
NFmiOracle::NFmiOracle()
    : test_mode_(false), verbose_(false), initialized_(false), pooled_connection_(false), credentials_set_(false){};
...
void NFmiOracle::Execute(const string& sql)
{
	...
	try
	{
		oracle::otl_cursor::direct_exec(db_, sql.c_str(), oracle::otl_exception::enabled);
	}
	catch (oracle::otl_exception& p)
	{
		if (verbose_)
...
string NFmiOracle::MakeDate(const otl_datetime& time)
{
	...
	snprintf(date, date_mask_.size() + 1, date_mask_.c_str(), time.year, time.month, time.day, time.hour, time.minute,
	         time.second);
